...
[trusted-types.js](trusted-types.js) registers a restrictive default Trusted Types policy (blocks HTML/script sinks; allows same-origin script URLs only) so React can run under the strict CSP in [security/csp.ts](../security/csp.ts). It must load synchronously in index.html before the module entry.
