...
| Migrate legacy `otherDietary` to `allergyDetails` | **Staged rollout (required):** (1) Deploy with legacy `otherDietary: v.optional(v.string())` still present in [applicationFields.ts](../convex/applicationFields.ts). (2) Run `npx convex run migrations:migrateOtherDietaryToAllergyDetails` (add `--prod` for production). Non-empty `allergyDetails` wins when both columns exist. (3) Remove `otherDietary` from `applicationRecord`, redeploy. Confirm `updated: 0` on a second migration run before step 3 in production. |
| Migrate legacy `firstHackathon` yes/no to `hackathonsAttended` | `npx convex run migrations:migrateFirstHackathonToHackathonsAttended` (add `--prod` for production). Maps `true` → `0`, `false` → `1`, then removes `firstHackathon`. If deploy fails schema validation because rows still have `firstHackathon`, temporarily add `firstHackathon: v.optional(v.boolean())` to [applicationFields.ts](../convex/applicationFields.ts), deploy, run migration, remove the legacy field, redeploy. |
| Migrate merged Other/self-describe answers to `other*` columns | `npx convex run migrations:migrateMergedOtherFieldsToSeparateColumns` (add `--prod` for production). Splits free text stored in `school`, `major`, `hearAbout`, or `gender` into sentinel + `otherSchool` / `otherMajor` / `otherHearAbout` / `otherGender`. |
| Migrate legacy code-of-conduct consent to `mlhCodeOfConductAgreed` | `npx convex run migrations:migrateLegacyCodeOfConductFields` (add `--prod` for production). Consolidates `codeOfConductAgreed` and interim `MLHcodeOfConductAgreed` into `mlhCodeOfConductAgreed`, then removes legacy columns. |
| Rename legacy agreement `*SubmittedAt` columns to `*At` | `npx convex run migrations:migrateLegacyAgreementSubmittedAtFields` (add `--prod` for production). Copies `sponsorSharingConsentSubmittedAt` → `sponsorSharingConsentAt` and `foodAllergyWaiverSubmittedAt` → `foodAllergyWaiverAgreedAt`, then removes legacy columns. After `updated: 0` on a second run, remove the legacy optional fields from [applicationFields.ts](../convex/applicationFields.ts) and redeploy. |
